...
_JSON_PRICE = re.compile(
    r'"(?:price|finalPrice|regularPrice|pricePerUnit|current_retail|listPrice|customerPrice)"\s*:\s*"?([0-9]{2,4}(?:\.[0-9]{2})?)"?',
    re.I,
)


def _price_context_junk(ctx: str) -> bool:
    t = ctx.lower()
    if "shipping" in t and ("order" in t or "over" in t or "with $" in t or "with $" in t):
        return True
    if "free standard shipping" in t or "shipping with" in t:
        return True
    return False
...
def extract_price(text: str, hint: float | None = None) -> float | None:
    ...
    found: list[float] = []
    blob = text or ""
    for m in re.finditer(r"\$\s*([0-9]{2,4}(?:\.[0-9]{2})?)", blob):
        try:
            val = float(m.group(1))
        except ValueError:
            continue
        if not (15 <= val <= 900):
            continue
        ctx = blob[max(0, m.start() - 48) : m.end() + 48]
        if _price_context_junk(ctx):
            continue
        found.append(val)
    for m in _JSON_PRICE.finditer(blob):
        try:
            val = float(m.group(1))
        except ValueError:
            continue
